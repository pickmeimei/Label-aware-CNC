...
def main(args):
    ...
    adj_s, feature_s, label_s, idx_tot_s, c, cn, adj_s_dense,A_s_num= search_s_datak3(dataset=args.source_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)

    adj_t, feature_t, label_t, idx_tot_t, b ,adj_t_dense,ct,ctn,A_t_num= search_t_datak3(dataset=args.target_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)
    ...
    for i in c:
        for j in c[i]:
            if A_s_num[i] < 10 and A_s_num[j] < 10:
                adj_s_dense[i][j] = 1
                adj_s_dense[j][i] = 1
                source_edge += 1
    ...
    for i in range(len(targethop)):
        ...
        if tarpred>=lamta and A_t_num[m]<10 and A_t_num[n]<10:
