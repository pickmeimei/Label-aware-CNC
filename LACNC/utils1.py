...
def do_iterc(emb_model1, cly_model, adj, feature, labels, idx , cal_f1=False, is_social_net=False):
    embs = emb_model1(idx, adj, feature)
    preds = cly_model(embs)
    if is_social_net:
        labels_idx = torch.argmax(labels[idx], dim=1)
...
def search_s_datak4(file_path="./Datasets", dataset='acmv9.mat', device='cpu', seed=123, is_blog=False):
    # load raw data
    data_path = file_path + '/' + dataset
    data_mat = sio.loadmat(data_path)
    adj = data_mat['network']
    ...
    for i in A_tail_list:
                    ...
                    for k in range(adj_dense.shape[0]):
                        if adj_dense[j][k] != 0 and adj_dense[k][j] != 0 and k != i:
                            if k not in A_nei2_list:
                                nei2_num = nei2_num + 1
                                A_nei2_list.append(k)
                                for z in range(adj_dense.shape[0]):
                                    if adj_dense[k][z] != 0 and adj_dense[z][k] != 0 and k != i and k != j:
                                        if z not in A_nei3_list:
                                            nei3_num = nei3_num + 1
                                            A_nei3_list.append(z)
                                            for p in range(adj_dense.shape[0]):
                                                if adj_dense[z][p] != 0 and adj_dense[p][z] != 0 and z != i and z != j and z != k:
                                                    if p not in A_nei4_list:
                                                        nei4_num = nei4_num + 1
                                                        A_nei4_list.append(p)
        ...
        a[i] = A_nei1_list#一阶邻居
        b[i] = A_nei2_list#二阶邻居
        b3[i] = A_nei3_list#三阶邻居
        b4[i] = A_nei4_list#四阶邻居
    print("找到")
    ...
    return adj.to(device), features.to(device), labels.to(device), idx_tot.to(device), c,cn,adj_dense,A_num



def search_t_datak4(file_path="./Datasets", dataset='acmv9.mat', device='cpu', seed=123, is_blog=False):
    #找到目标域的尾节点的二跳节点
    # load raw data
    data_path = file_path + '/' + dataset
    data_mat = sio.loadmat(data_path)
...
def search_s_datak3(file_path="./Datasets", dataset='acmv9.mat', device='cpu', seed=123, is_blog=False):
    # load raw data
    data_path = file_path + '/' + dataset
    data_mat = sio.loadmat(data_path)
    adj = data_mat['network']
...
def search_t_datak3(file_path="./Datasets", dataset='acmv9.mat', device='cpu', seed=123, is_blog=False):
    #找到目标域的尾节点的二跳节点
    # load raw data
    data_path = file_path + '/' + dataset
    data_mat = sio.loadmat(data_path)
