...
def main(args):
    ...
    for i in c:
        for j in c[i]:
            if A_s_num[i] < 6 and A_s_num[j] < 6:
                adj_s_dense[i][j] = 1
                adj_s_dense[j][i] = 1
                source_edge += 1
    ...
    n=len(adj_s_dense)
    for i in range(n):
        for j in range(i, n):  # 只需遍历到j=i，以避免重复计数
            if adj_s_dense[i][j] != 0:
                edge_count_s += 1

    print("源域增强后边数",  edge_count_s)
    degree_s = np.sum(adj_s_dense, 1)# 计算节点的度
    # 统计度数和并打印
    sum_degrees_s = np.sum(degree_s)
    print("源域度数和:", sum_degrees_s)
    # 统计节点的度数小于9的节点数，并打印
    s_num_nodes_less_9 = np.sum(degree_s < 9)
    print("源域度数小于9的节点数:", s_num_nodes_less_9)
    ...
    for i in range(len(targethop)):
        ...
        if tarpred>=lamta and A_t_num[m]<9 and A_t_num[n]<9:
            ######是否要加一个度不超过多少的条件 if tarpred>=lamta and A_t_num[m]<7 and A_t_num[n]<7:
            #####加入阈值小于0.5的节点对，即标签不同的节点对
    ...
    print("目标域添加的边数:", target_edge)
    n1 = len(adj_t_dense)
    for i in range(n1):
        for j in range(i, n1):  # 只需遍历到j=i，以避免重复计数
            if adj_t_dense[i][j] != 0:
                edge_count_t += 1
    print("目标域增强后边数", edge_count_t)
    degree_t = np.sum(adj_t_dense, 1)  # 计算节点的度
    # 统计度数和并打印
    sum_degrees_t = np.sum(degree_t)
    print("目标域度数和:", sum_degrees_t)
    # 统计节点的度数小于9的节点数，并打印
    t_num_nodes_less_9 = np.sum(degree_t < 9)
    print("目标域度数小于9的节点数:", t_num_nodes_less_9)
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(num_batch):
            ...
            target_features, _ = do_iter(emb_model, cly_model, adj_t, feature_t, label_t, idx=b_nodes_t,
                                         is_social_net=args.is_social_net)
            # source_features, cly_loss_s, cluster_loss_s = do_iter3(emb_model, cly_model, adj_s, feature_s, label_s,
            #                                                       idx=b_nodes_s,
            #                                                       is_social_net=args.is_social_net)
            # target_features, _, _ = do_iter3(emb_model, cly_model, adj_t, feature_t, label_t, idx=b_nodes_t,
            #                                 is_social_net=args.is_social_net)
            shuf_idx_s = np.arange(label_s.shape[0])
            np.random.shuffle(shuf_idx_s)
            shuf_feat_s = feature_s[shuf_idx_s, :]
