...
def main(args):
    ...
    for i in c:
        for j in c[i]:
            if A_s_num[i] < 7 and A_s_num[j] < 7:
                adj_s_dense[i][j] = 1
                adj_s_dense[j][i] = 1
                source_edge += 1
    ...
    for i in range(len(targethop)):
        ...
        if tarpred>=lamta and A_t_num[m]<6 and A_t_num[n]<6:
...
#加载预训练后的数据
    ...
    for epoch in range(args.epochs):
        ...
        if (micro_f1_t + macro_f1_t) > (best_micro_f1 + best_macro_f1):
            ...
            best_epoch = epoch
            scipy.io.savemat('./' + emb_filename + '_emb_LACNC.mat',
                                             {'rep_S': embs_whole_s, 'rep_T': embs_whole_t,'label_S':targets_whole_s,'label_T':targets_whole_t})
            print('saving model...')
        scheduler.step()
