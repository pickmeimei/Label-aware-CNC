...
def main(args):
    ...
    adj_s, feature_s, label_s, idx_tot_s, c, c1 ,cn,cn1 ,adj_s_dense,A_s_num= search_s_data(dataset=args.source_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)

    adj_t, feature_t, label_t, idx_tot_t, b ,adj_t_dense,ct,ctn,A_t_num= search_t_data(dataset=args.target_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)
    ...
    # embs_whole_s=(embs_whole_s+0.00001)*10000
    # embs_whole_t = (embs_whole_t + 0.00001) * 10000
...
##训练一个MLP，看看其鉴别能力怎么样
    ...
    en = torch.tensor(en)
    lenneg=len(en)
    # net = Net3(384,1024,512,256,128,128,256,512,1024);##########
    net = Net2(384, 256, 128, 64, 32)
    ...
    net.eval()
    print(loss_all)
    print("本轮阈值平均值：" + str(lamta1))
    print("本轮设定的阈值：" + str(lamta))
    ...
    for i in range(len(targethop)):
        ...
        # if (label_t[m] == label_t[n]).all():
        #     adj_t_dense[targethop[i][1]][targethop[i][0]] = 1
        #     adj_t_dense[targethop[i][0]][targethop[i][1]] = 1
        #     rightnumber = rightnumber + 1


        if tarpred>=lamta:######是否要加一个度不超过多少的条件
...
#加载数据
    ...
    adj_s, feature_s, label_s, idx_tot_s = load_data_s(dataset=args.source_dataset + '.mat', device=device,
                                                        seed=args.seed, is_blog=args.is_blog, adj_s_dense=adj_s_dense)
    adj_t, feature_t, label_t, idx_tot_t = load_data_t(dataset=args.target_dataset + '.mat', device=device,
                                                             seed=args.seed, is_blog=args.is_blog, adj_t_dense=adj_t_dense)



#重新训练,第二部分把adj_s,和adj_t改成了adj_s_dense,adj_t_dense即用新的邻接矩阵A
