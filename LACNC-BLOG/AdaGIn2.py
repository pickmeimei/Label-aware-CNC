...
def main(args):
    ...
    for epoch in range(args.epochs1):
        ...
        for iter in range(num_batch):
            ...
            source_features, cly_loss_s = do_iter2(emb_model1,cly_model, adj_s, feature_s,label_s, idx=b_nodes_s,
                                        is_social_net=args.is_social_net)
            target_features, _ = do_iter2(emb_model1, cly_model, adj_t, feature_t,label_t, idx=b_nodes_t,
                                        is_social_net=args.is_social_net)
            #dgi扰动进行了增强，可能能缓解稀疏性
        ...
        embs_whole_s, targets_whole_s = evaluate2(emb_model1, cly_model, adj_s,feature_s, label_s,idx_tot_s, args.batch_size,
                                                                                         mode='test',is_social_net=args.is_social_net)




        embs_whole_t, targets_whole_t = evaluate2(emb_model1, cly_model, adj_t,feature_t, label_t, idx_tot_t, args.batch_size,
                                                                                         mode='test',is_social_net=args.is_social_net)
...
###################################################
    ...
    for i in range(len(targethop1)):
        ...
        if tarpred>=lamta and A_t_num[m]<16 and A_t_num[n]<16:
            ######是否要加一个度不超过多少的条件
