...
def main(args):
    ...
    adj_t, feature_t, label_t, idx_tot_t, b ,adj_t_dense,ct,ctn,A_t_num, A_head, A_tail = search_t_data_tail(dataset=args.target_dataset + '.mat', device=device,
                                                     seed=args.seed, is_blog=args.is_blog)
    ...
    for i in c:
        for j in c[i]:
            if A_s_num[i] < 5 and A_s_num[j] < 5:
                adj_s_dense[i][j] = 1
                adj_s_dense[j][i] = 1
                source_edge += 1
    ...
    for i in range(len(targethop)):
        ...
        if tarpred>=lamta and A_t_num[m]<7 and A_t_num[n]<7:
...
#加载预训练后的数据
    ...
    for epoch in range(args.epochs):
        ...
        cly_loss_bat_t, micro_f1_t, macro_f1_t, tail_micro_f1_t, tail_macro_f1_t, head_micro_f1_t, head_macro_f1_t, embs_whole_t, targets_whole_t = evaluate_tail(
            emb_model, cly_model,adj_t, feature_t, label_t, idx_tot_t, args.batch_size, mode='test', is_social_net=args.is_social_net,
            A_head_list=A_head, A_tail_list=A_tail)
